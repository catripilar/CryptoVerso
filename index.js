...
async function conectar() {
    if (conectado == false){
        ...
        if (typeof window.ethereum !== 'undefined') {
            ...
            const nfts = await contract.methods.NFTData().call();
            for (var i = 0; i < nfts.length; i++) {
                if(nfts[i].owner == contas[0]){
                    ...
                    document.getElementById("expire").textContent = `data de expiração: ${formatUnixTime(nfts[i].expired)}`;
                }
            }
            if (infoParam) {
            ...
            if (tokenId != 0){
                document.getElementById("sec_owner").style.display = "block";
            }
        }
    }
}
async function burn() {
    if (conectado == true && tokenId != 0 && ID != 0){
        const web3 = new Web3(window.ethereum);
        const contas = await web3.eth.getAccounts();
        const contract = await new web3.eth.Contract(abi,contrato_addr);
        await contract.methods.burning(tokenId).send({from: contas[0]})
        .then(_ => {ID = id_discord;alert("NFT queimada com sucesso!")})
        .catch(_ => {alert("erro ao queimar sua NFT..")})
    }
}
async function ativar() {
    const id_discord = document.getElementById("discordId").value;
    if (conectado == true && tokenId != 0 && ID == 0){
        const web3 = new Web3(window.ethereum);
        const contas = await web3.eth.getAccounts();
        const contract = await new web3.eth.Contract(abi,contrato_addr);
    ...
    if (carteira != ""){
        if (conectado == true){
            ...
            if (free == true && time < 7){
                ...
                .then(_ => {ID = id_discord;alert("NFT mintada com sucesso!")})
                .catch(_ => {alert("erro ao mintar sua NFT..")})
            }else{
